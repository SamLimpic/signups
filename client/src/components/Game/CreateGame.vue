  ...
  <form :class="{'mt-md-5 mt-4 mb-md-3': indexProp !== 0}" class="row justify-content-around bg-light rounded shadow p-3">
    <div class="col-lg-6 col-12">
      <div class="form-group font-md">
        ...
        <input type="text" class="form-control font-sm" placeholder="I expect puns..." v-model="state.liveGames[indexProp].title">
      </div>
      <div class="form-group font-md">
        <label>Game Description</label>
        <textarea class="form-control font-sm" rows="3" placeholder="Hit me with that purple prose!" v-model="state.liveGames[indexProp].description"></textarea>
      </div>
    </div>
    <div class="col-lg-6 col-12">
      <div class="row justify-content-around">
        <div class="form-group col-lg-5 col-sm-4 col-4 font-md">
          <label>Weeks</label>
          <select class="form-control font-sm" v-model="state.liveGames[indexProp].length">
            <option>1</option>
            <option>2</option>
            <option>3</option>
            <option>4</option>
        ...
        <div class="form-group col-lg-7 col-sm-4 col-8 font-md">
          ...
          <select class="form-control font-sm" v-model="state.liveGames[indexProp].experience">
        ...
        <div class="form-group col-lg-5 col-sm-4 col-4 font-md">
          ...
          <input type="number"
                 ...
                 max="20"
                 placeholder="5"
                 v-model="state.liveGames[indexProp].size"
          >
        </div>
        <div class="form-group col-lg-7 col-md-6 col-sm-6 col-8 font-md">
          <label>{{ state.liveGames[indexProp].day }}</label>
          <input type="date"
                 class="form-control font-sm"
                 v-model="state.liveGames[indexProp].date"
          >
        </div>
        <div class="form-group col-lg-7 col-md-6 col-sm-6 col-5 text-left font-md pl-lg-3 pl-md-4 pl-sm-2 pl-3 pr-0 pt-lg-0 pt-sm-4 mb-0">
          <div class="form-check form-check-inline m-0 pr-md-3 pr-md-3 pr-sm-2 pr-2 mt-lg-2 mt-sm-3 pt-sm-0 pt-1">
            <input class="form-check-input ml-md-0 ml-sm-2 ml-0" type="checkbox" v-model="state.liveGames[indexProp].outdoor">
            <label class="form-check-label">Outdoor</label>
          </div>
          <div class="form-check form-check-inline m-0 mt-lg-2 mt-sm-3 mt-0">
            <input class="form-check-input ml-md-0 ml-sm-2 ml-0" type="checkbox" v-model="state.liveGames[indexProp].masked">
            <label class="form-check-label">Masked</label>
          </div>
        </div>
        <div class="form-group col-lg-5 col-md-6 col-7 text-sm-right text-center mt-lg-auto pb-sm-2 mt-sm-3 mt-auto mb-2" v-if="state.liveGames[indexProp].date">
          <button type="button" class="btn btn-success font-xs w-100" @click="addGame(state.liveGames[indexProp])" v-if="state.liveGames[indexProp].date">
            CONFIRM
          </button>
    ...
    return {
      ...
      async addGame(game) {
        try {
          if (state.account.dm) {
            if (game.id) {
              await gamesService.editGame(game)
